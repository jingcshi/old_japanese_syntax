...
from .step3 import PolicyRoute3, PolicyDNF3


class AtomicRequirement3B(BaseModel):
    """Style B: Atomic requirement with category label"""
    model_config = ConfigDict(extra='forbid', validate_assignment=True)

    分类: Literal["年龄性别", "原有户籍", "住房居住", "就业", "投资", "纳税",
                "学历", "职业资格", "社保", "亲属投靠", "奖励荣誉", "其他"] = Field(default="其他")
    要求: str = Field(default="")


class PolicyRoute3B(BaseModel, ListDedupeMixin):
    """
    Step 3 variant of PolicyRouteB where region is modelled as free text.
    """
    model_config = ConfigDict(extra='forbid', validate_assignment=True)

    适用地区: str = Field(default="")
    落户要求: List[AtomicRequirement3B] = Field(default_factory=list, description="该落户渠道中包含的具体要求（按照给定分类记录）。申请者满足其中所有的具体要求才视作满足此渠道")
    ...
    @classmethod
    def validate_unique_categories(cls, values: Any, handler: Any) -> 'PolicyRoute3B':
    ...
    def convert_to_unified(self) -> PolicyRoute3:
...
class PolicyDNF3B(BaseModel, ListDedupeMixin):
    """Style B: Policy document with flexible list format"""
    model_config = ConfigDict(extra='forbid', validate_assignment=True)

    落户渠道: List[PolicyRoute3B] = Field(default_factory=list, description="将落户政策拆分为若干并列的渠道，申请者满足任意一渠道中的所有要求即可落户")
    ...
    def convert_to_unified(self) -> PolicyDNF3:
        """Convert Style B policy document to unified DNF"""
        routes = [route.convert_to_unified() for route in self.落户渠道]
        return PolicyDNF3(落户渠道=routes)
