...
class Requirement3A(BaseModel):
    ...
    亲属投靠: Optional[str] = Field(default=None)
    奖励荣誉: Optional[str] = Field(default=None)
    其他: Optional[str] = Field(default=None)

    def clean_placeholder_values(self) -> 'Requirement3A':
...
class PolicyRoute3A(BaseModel):
    """Step 3 variant of PolicyRouteA where region is modelled as free text."""
    model_config = ConfigDict(extra='forbid', validate_assignment=True)

    适用地区: str = Field(default="")
    落户要求: Requirement3A = Field(
        default_factory=Requirement3A,
...
class PolicyDNF3A(BaseModel, ListDedupeMixin):
    """Style A: Policy document with fixed schema"""
    model_config = ConfigDict(extra='forbid', validate_assignment=True)

    落户渠道: List[PolicyRoute3A] = Field(default_factory=list, description="将落户政策拆分为若干并列的渠道，申请者满足任意一渠道中的所有要求即可落户")
